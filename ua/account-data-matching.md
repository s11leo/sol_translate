...
Для початку завантажте стартовий код з гілки `starter` [цього репозиторію](https://github.com/Unboxed-Software/solana-account-data-matching). Стартовий код включає програму з двома інструкціями та заготовку для тестового файлу.
...
Додайте тест для виклику інструкції `insecure_withdraw`. Використовуйте `withdrawDestinationFake` як обліковий запис виводу та `walletFake` як авторитет. Потім відправте транзакцію за допомогою `walletFake    .
...
Якщо ви хочете переглянути код остаточного рішення, ви можете знайти його на гілці `solution` [репозиторію](https://github.com/Unboxed-Software/solana-account-data-matching/tree/solution).
